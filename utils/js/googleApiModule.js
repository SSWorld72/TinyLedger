        ...
        container.innerHTML = `
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="input-${this.containerId}-key" 
                    placeholder="輸入 API Key (選填)" style="width: 100%; padding: 8px; border: 1px solid var(--border-color); border-radius: 4px; background: var(--bg-color); color: var(--text-main);">
            </div>
        `;
                ...
                <div style="max-width: 520px; width: 92%; background: var(--bg-color); border-radius: 12px; box-shadow: 0 8px 32px rgba(0,0,0,0.2); overflow: hidden; display: flex; flex-direction: column;">
                    ...
                    <div style="padding: 20px; font-size: 0.9rem; line-height: 1.7; color: var(--text-main); overflow-y: auto; max-height: 70vh;">
                        <p style="margin: 0 0 12px 0; color: var(--text-muted);">
                            本功能需要一組 <strong style="color: var(--text-main);">Google Cloud API 金鑰</strong>，
                            用於啟用地圖相關服務（如地點搜尋、自動完成等）。金鑰僅儲存於您的瀏覽器本機，不會上傳至任何伺服器。
                        </p>
                        <hr style="border: none; border-top: 1px solid var(--border-color); margin: 16px 0;">

                        <div style="margin-bottom: 16px;">
                            <strong style="display: inline-block; background: var(--active-bg); color: var(--primary-color); padding: 2px 8px; border-radius: 4px; font-size: 0.8rem; margin-bottom: 8px;">第一階段：建立 Google Cloud 專案</strong>
                            <ol style="margin: 4px 0 0 0; padding-left: 20px;">
                                <li style="margin-bottom: 6px;">前往 <a href="https://console.cloud.google.com/" target="_blank" style="color: var(--primary-color); text-decoration: underline; font-weight: 600;">Google Cloud Console</a>，使用您的 Google 帳號登入。</li>
                                <li style="margin-bottom: 6px;">點擊頂部導覽列的「<strong style="color: var(--text-main);">選取專案</strong>」下拉選單，再點擊右上角的「<strong style="color: var(--text-main);">新增專案</strong>」。</li>
                                <li style="margin-bottom: 6px;">輸入一個容易辨識的專案名稱（例如：<code style="background: var(--surface-color); padding: 1px 6px; border-radius: 3px; font-size: 0.85em; color: var(--primary-color);">My Map App</code>），然後按「建立」。</li>
                                <li style="margin-bottom: 6px;">建立完成後，確認目前已選擇到該專案（頂部應顯示您剛建立的專案名稱）。</li>
                            </ol>
                        </div>

                        <div style="margin-bottom: 16px;">
                            <strong style="display: inline-block; background: var(--active-bg); color: var(--primary-color); padding: 2px 8px; border-radius: 4px; font-size: 0.8rem; margin-bottom: 8px;">第二階段：啟用 API 並建立金鑰</strong>
                            <ol style="margin: 4px 0 0 0; padding-left: 20px;">
                                <li style="margin-bottom: 6px;">在左側選單中點擊「<strong style="color: var(--text-main);">API 和服務</strong>」>「<strong style="color: var(--text-main);">程式庫</strong>」。</li>
                                <li style="margin-bottom: 6px;">在搜尋框中逐一搜尋以下 API，點進去後按「<strong style="color: var(--primary-color);">啟用</strong>」：<br>${requiredApisHtml ? requiredApisHtml : '<strong style="color: var(--primary-color);">對應的 API</strong>'}。</li>
                                <li style="margin-bottom: 6px;">回到左側選單，點擊「<strong style="color: var(--text-main);">API 和服務</strong>」>「<strong style="color: var(--text-main);">憑證</strong>」。</li>
                                <li style="margin-bottom: 6px;">點擊頂部的「<strong style="color: var(--text-main);">+ 建立憑證</strong>」>「<strong style="color: var(--primary-color);">API 金鑰</strong>」，系統會自動產生一組金鑰。</li>
                                <li style="margin-bottom: 6px;">在彈出的視窗中，先<strong style="color: #ea4335;">複製</strong>這組金鑰（之後要貼回來）。</li>
                            </ol>
                        </div>

                        <div style="margin-bottom: 16px;">
                            <strong style="display: inline-block; background: var(--active-bg); color: var(--primary-color); padding: 2px 8px; border-radius: 4px; font-size: 0.8rem; margin-bottom: 8px;">第三階段：（建議）限制金鑰 + 貼回設定</strong>
                            <ol style="margin: 4px 0 0 0; padding-left: 20px;">
                                <li style="margin-bottom: 6px;">在剛才的彈出視窗中，點擊「<strong style="color: var(--text-main);">編輯 API 金鑰</strong>」（或在憑證列表中點該金鑰的鉛筆圖示）。</li>
                                <li style="margin-bottom: 6px;">在「<strong style="color: var(--text-main);">應用程式限制</strong>」區塊選擇「<strong style="color: var(--text-main);">網站</strong>」，並點擊「<strong style="color: var(--primary-color);">+ Add</strong>」加入 <code>https://ssworld72.github.io/*</code> 以允許正式版存取。<br><span style="font-size: 0.85rem; color: var(--text-muted);">(註：若您有在本地端開發的需求，可再額外加入 <code>http://127.0.0.1/*</code> 與 <code>http://localhost/*</code>)</span></li>
                                <li style="margin-bottom: 6px;">向下捲動到「<strong style="color: var(--text-main);">API 限制</strong>」區塊，選擇「<strong style="color: var(--text-main);">限制金鑰</strong>」，然後下拉勾選您剛才啟用的 API 進行綁定。</li>
                                <li style="margin-bottom: 6px;">按下「<strong style="color: var(--primary-color);">儲存</strong>」即可。</li>
                                <li style="margin-bottom: 6px;">最後，將複製好的金鑰貼回本頁的「<strong style="color: var(--text-main);">${this.apiName} Key</strong>」輸入框中，大功告成！</li>
                            </ol>
                        </div>
                    ...
                    <div style="display: flex; justify-content: flex-end; padding: 12px 20px; border-top: 1px solid var(--border-color); background: var(--surface-color);">
                        <button id="btn-ok-${this.containerId}-help" style="padding: 8px 20px; font-weight: 600; background: var(--primary-color); color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 0.9rem; transition: filter 0.2s;" onmouseover="this.style.filter='brightness(1.1)'" onmouseout="this.style.filter='brightness(1)'">了解</button>
                    </div>
                </div>
            `;
