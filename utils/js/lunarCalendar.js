 ...
 * 
 * lunarCalendar.js — Taiwan Farmer's Almanac Module (Perpetual Calendar)
 * Provides Solar ↔ Lunar conversion, Stems & Branches, 24 Solar Terms, Bazi (Four Pillars), Festival querying, and National Holiday integration.
 * Supported range: 1900-01-31 to 2100-12-31.
 * 
 ...
 * 
 * ── Module Dependencies ──
 * This file depends on taiwanHolidays.js in the same directory.
 * When porting to other projects, both files must be copied to work properly.
 * Other than this, it has no external dependencies and serves purely as a shared utility module.
 * 
 * @example
 ...
 * 
 *  ⚠️ Accuracy Limitation Declaration
 * ──────────────────────────────────────────────────────────────
 *  Positioned as a lightweight reference, lacking precise ephemeris algorithms.
 *  1. Solar Term accuracy is up to 'Day', without precise hour/minute handovers.
 *     Defaults to switching Stems & Branches at 00:00 on handover days.
 *  2. Bazi: Year pillar bounded by LiChun, Month pillar by solar terms, all at day-level accuracy.
 *  3. Early/Late Zi hour: Automatically advances day pillar at 23:00 (Early/Late Zi share the same heavenly stem).
 * ══════════════════════════════════════════════════════════════
 * 
 * ── 公開 API 摘要 ──
 * 
 ...
 * [Core Conversion]
 *   gregorianToLunar(y, m, d, skipFestivals?)
 *     → Gregorian to Lunar conversion, returns full lunar info object
 * 
 * [Bazi Layout]
 *   getBazi(y, m, d, hour?)
 *     → Returns four pillars layout including ten gods, hidden stems, and nayin
 *     → If hour=23, additionally provides earlyZiBazi (Early Zi hour layout)
 * 
 * 【節日查詢】
...
import {
    getGlobalSolarFestival,
    getGlobalWeekFestival,
    getEasterFestival
} from './globalFestivals.js';

// ===== 陰曆年資訊 compact 編碼 (1900–2100) =====
// ===== Lunar Year Compact Encoding (1900-2100) =====
// 16-bit 編碼，跟經典 lunisolar 演算法一致
// 16-bit encoding, consistent with classic lunisolar algorithm
const lunarInfo = [
...
// 邊界防護檢查 (1900 - 2100)
// Boundary Protection Check (1900 - 2100)
function assertLunarYear(y) {
    if (!Number.isInteger(y) || y < 1900 || y > 2100) {
        throw new Error(`農曆年份超出支援範圍 (1900-2100): ${y} / Lunar year out of supported range (1900-2100): ${y}`);
    }
...
function leapDays(y) {
    ...
    if (nextYearIndex >= lunarInfo.length) {
        ...
        // [Boundary Protection]: Prevent out-of-bounds access if supported range expands in the future. Year 2100 has no leap month (leap = 0) so it won't hit this logic.
        throw new Error(`缺少 ${y} 年閏月大小資料 / Missing leap month data for year ${y}`);
    }

    // [修正紀錄]: lunarInfo 為 16-bit 壓縮，無 0x10000 位元，必須使用原版 Sean Lin 演算法
    // [Fix Record]: lunarInfo is 16-bit compressed and lacks the 0x10000 bit; must use the original Sean Lin algorithm.
    // 若下一年 leapMonth 原始值為 0xf（即無閏月），代表本年閏月 = 30 天；否則 29 天
    // If the next year's raw leapMonth value is 0xf (no leap month), it means this year's leap month = 30 days; otherwise 29 days.
    return ((lunarInfo[nextYearIndex] & 0xf) === 0xf) ? 30 : 29;
}
function leapMonth(y) {
    assertLunarYear(y);
    // 經典 16-bit 表格中，低 4 位元代表閏月月份。
    // In the classic 16-bit table, the lower 4 bits represent the leap month.
    // 若值為 0 代表無閏月；若值為 0xf，不僅代表無閏月，同時暗示上一年閏月為大月 (30天)。
    // If the value is 0, it means no leap month; if 0xf, it means no leap month AND implies the previous year's leap month was 30 days.
    // 因此對外回傳時，0xf 應視為 0（無閏月）。
    // Therefore, when returning externally, 0xf should be treated as 0 (no leap month).
...
function monthDays(y, m) {
    ...
    if (!Number.isInteger(m) || m < 1 || m > 12) {
        throw new Error(`農曆月份無效: ${m} / Invalid Lunar month: ${m}`);
    }
    return (lunarInfo[y - 1900] & (0x10000 >> m)) ? 30 : 29;
}
function solarDays(y, m) {
    if (!Number.isInteger(m) || m < 1 || m > 12) {
        throw new Error(`國曆月份無效: ${m} / Invalid Solar month: ${m}`);
    }
    if (m === 2) {
...
// ===== 節氣自動計算（天文常數標準算法） =====
// ===== Solar Term Auto Calculation (Astronomical Constants Standard Algorithm) =====
const sTermInfo = [
...
export function gregorianToLunar(y, m, d, skipFestivals = false) {
    if (!Number.isInteger(y) || y < 1900 || y > 2100) throw new Error('年份必須介於 1900 至 2100 之間且為整數 / Year must be an integer between 1900 and 2100');
    if (!Number.isInteger(m) || m < 1 || m > 12) throw new Error('月份必須介於 1 至 12 之間且為整數 / Month must be an integer between 1 and 12');
    if (!Number.isInteger(d)) throw new Error('日期必須為整數 / Day must be an integer');
    const _daysInMonth = new Date(Date.UTC(y, m, 0)).getUTCDate();
    if (d < 1 || d > _daysInMonth) throw new Error('無效的日期 / Invalid date');

    const minDate = Date.UTC(1900, 0, 31);
    const maxDate = Date.UTC(2100, 11, 31);
    const inputDate = Date.UTC(y, m - 1, d);
    if (inputDate < minDate || inputDate > maxDate) throw new Error('日期必須介於 1900-01-31 至 2100-12-31 之間 / Date must be between 1900-01-31 and 2100-12-31');
    ...
    if (offset > 0) {
        throw new Error('農曆資料不足，無法轉換此日期 (超過 2100 年邊界) / Lunar data insufficient, unable to convert this date (exceeds year 2100 boundary)');
    }
    ...
    // ==========================================
    // 6. 整合所有節慶（農曆、國曆、節氣、節日）
    // 6. Integrate all festivals (Lunar, Gregorian, Solar Terms, Festivals)
    // ==========================================
    let allFestivals = [];
    let lunarFestivalsArr = [];
    let globalFestivalsArr = [];
    let combinedFestival = null;
    let combinedLunarFestival = null;
    let combinedGlobalFestival = null;
    
    if (!skipFestivals) {
        // 農曆節日
        // Lunar Festivals
        const lunarFest = getLunarFestival(displayMonth, lunarDay, isLeap);
        if (lunarFest) { allFestivals.push(lunarFest); lunarFestivalsArr.push(lunarFest); }
        
        // 處理除夕 (臘月最後一天)
        // Handle Lunar New Year's Eve (Last day of the 12th lunar month)
        ...
        if (isLastMonthOfYear && lunarDay === currentMonthDays) {
            allFestivals.push('除夕'); lunarFestivalsArr.push('除夕');
        }
        
        // 處理關鬼門 (七月最後一天)
        // Handle Ghost Gates Closing (Last day of the 7th lunar month)
        const isLastMonthOf7 = (leap === 7) ? (displayMonth === 7 && isLeap) : (displayMonth === 7 && !isLeap);
        if (isLastMonthOf7 && lunarDay === currentMonthDays) {
            allFestivals.push('地藏王菩薩聖誕 關鬼門'); lunarFestivalsArr.push('地藏王菩薩聖誕 關鬼門');
        }
        
        // 國曆節日
        // Gregorian/Solar Festivals (Local)
        const solarFest = getSolarFestival(m, d);
        if (solarFest) { allFestivals.push(solarFest); lunarFestivalsArr.push(solarFest); }
        
        // 國際節日
        // Global Solar Festivals
        const globalSolarFest = getGlobalSolarFestival(m, d);
        if (globalSolarFest) { allFestivals.push(globalSolarFest); globalFestivalsArr.push(globalSolarFest); }


        // 第幾週節日 (例如母親節)
        // Global Week-based Festivals (e.g., Mother's Day)
        const globalWeekFest = getGlobalWeekFestival(y, m, d);
        if (globalWeekFest) { allFestivals.push(globalWeekFest); globalFestivalsArr.push(globalWeekFest); }
        
        // 節氣轉節日 (清明)
        // Convert Solar Term to Festival (e.g., Qingming Festival)
        if (termName === '清明') { allFestivals.push('清明節'); lunarFestivalsArr.push('清明節'); }
        
        // 復活節相關
        // Easter-related Festivals
        const easterFest = getEasterFestival(y, m, d);
        if (easterFest) { allFestivals.push(easterFest); globalFestivalsArr.push(easterFest); }
        
        // 去除重複並合併
        // Remove duplicates and merge
        combinedFestival = allFestivals.length > 0 ? [...new Set(allFestivals)].join(' ') : null;
        combinedLunarFestival = lunarFestivalsArr.length > 0 ? [...new Set(lunarFestivalsArr)].join(' ') : null;
        combinedGlobalFestival = globalFestivalsArr.length > 0 ? [...new Set(globalFestivalsArr)].join(' ') : null;
    }
    ...
    return {
        ...
        lunarFestivalStr: combinedLunarFestival,
        globalFestivalStr: combinedGlobalFestival,
...
/**
 * 【農曆節日資料庫】
 * 這些是中國傳統的節日，因此統一使用繁體中文顯示，不進行多語系翻譯。
 * 
 * [Lunar Festivals Database]
 * These are Traditional Chinese festivals, therefore they are uniformly displayed in Traditional Chinese and not subjected to multi-language translation.
 */
...
/**
 * 【國曆節日資料庫】
 * 這些是中國傳統的節日（包含台灣在地紀念日），因此統一使用繁體中文顯示，不進行多語系翻譯。
 * （全球通用節日請至 globalFestivals.js 設定）
 * 
 * [Solar Festivals Database]
 * These are Traditional Chinese festivals (including local Taiwan memorial days), therefore they are uniformly displayed in Traditional Chinese and not subjected to multi-language translation.
 * (For global international festivals, please refer to globalFestivals.js)
 */
const solarFestivals = [
    { month: 1, day: 1, name: '中華民國開國紀念日', isHoliday: true },
    ...
    { month: 10, day: 31, name: '蔣公誕辰紀念日 榮民節' },
    ...
    { month: 12, day: 25, name: '行憲紀念日 民族復興節' },
...
/**
 * 【十神名稱資料庫】
 * 這些是中國傳統八字命理（十神）的專有名詞，因此統一使用繁體中文顯示，不進行多語系翻譯。
 * 
 * [Ten Gods Database]
 * These are proper nouns used in Traditional Chinese Bazi (Four Pillars) astrology, therefore they are uniformly displayed in Traditional Chinese and not subjected to multi-language translation.
 */
const tenGodNames = [
...
/**
 * 【地支藏干資料庫】
 * 這些是中國傳統八字命理（地支藏干）的專有名詞，因此統一使用繁體中文顯示，不進行多語系翻譯。
 * 
 * [Hidden Stems Database]
 * These are proper nouns used in Traditional Chinese Bazi (Four Pillars) astrology for hidden heavenly stems, therefore they are uniformly displayed in Traditional Chinese and not subjected to multi-language translation.
 */
const hiddenStems = {
    '子': ['癸'],
    '丑': ['己', '癸', '辛'],
...
/**
 * 【六十甲子納音資料庫】
 * 這些是中國傳統八字命理（納音）的專有名詞，因此統一使用繁體中文顯示，不進行多語系翻譯。
 * 
 * [Sexagenary Cycle Nayin Database]
 * These are proper nouns used in Traditional Chinese Bazi (Four Pillars) astrology for Nayin (Melodic Elements), therefore they are uniformly displayed in Traditional Chinese and not subjected to multi-language translation.
 */
const nayinTable = {
...
function buildBaziFromGanZhi(yearGz, monthGz, dayGz, _yearGzIdx, _monthGzIdx, dayGzIdx, hour, advanceDayGanForHour = false) {
    ...
    if (yearG < 0 || monthG < 0 || dayG < 0) {
        throw new Error('無效的干支字串 / Invalid Stems and Branches string');
    }
    ...
    if (hour !== undefined) {
        if (!Number.isInteger(hour) || hour < 0 || hour > 23) {
            throw new Error('hour 必須是 0 至 23 的整數 / hour must be an integer between 0 and 23');
        }
 ...
 * 
 * Get complete Bazi layout info (Public utility function)
 * Includes four pillars, ten gods, hidden stems, sexagenary nayin
 * 
 * ⚠️ Precision Limitation Note:
 * Lightweight reference, lacks precise ephemeris.
 * 1. Solar term boundary is day-level. No precise hour:minute. Default 00:00 boundary on handover days.
 * 2. 23:00 automatically advances day pillar.
 * 
 * @param {number} y - 國曆年份
 * @param {number} m - 國曆月份
 * @param {number} d - 國曆日期
 ...
 * @param {number} m - Gregorian Month
 * @param {number} d - Gregorian Day
 * @param {number} [hour] - Birth hour (0-23), undefined if not provided
 * @returns {Object} Bazi detailed info object
 */
export function getBazi(y, m, d, hour) {
    ...
    const minDate = Date.UTC(1900, 0, 31);
    const inputDate = Date.UTC(y, m - 1, d);
    if (inputDate < minDate) throw new Error('日期必須介於 1900-01-31 至 2100-12-31 之間 / Date must be between 1900-01-31 and 2100-12-31');

    if (hour !== undefined && (!Number.isInteger(hour) || hour < 0 || hour > 23)) {
        throw new Error('出生時間必須是 0 至 23 的整數 / Birth hour must be an integer between 0 and 23');
    }
    ...
    // Correct reverse calculation formula for sexagenary cycle index (same as monthGzIdx)
    ...
    // Early/Late Zi hour dual output logic: if 23:00, generate extra 'Early Zi Hour (Day pillar carries to tomorrow)' Bazi result
    if (hour === 23) {
        // 利用 Date.UTC 取得明天日期（自動處理跨月跨年）
        // Use Date.UTC to get tomorrow's date (auto-handles cross-month and cross-year)
        ...
        if (nextY <= 2100) {
            ...
            // Pass hour = 0 and advanceDayGanForHour = false, because for early Zi hour (tomorrow), it is tomorrow's Zi hour (00:00)
            ...
            // If 2100-12-31 23:00 pushes to 2101-01-01 causing out-of-bounds, ignore early Zi hour day pillar calculation
