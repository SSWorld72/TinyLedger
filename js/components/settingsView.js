...
function getCompletePreferences() {
    ...
    return {
        ...
        photoUploadEnabled: localStorage.getItem('tinyledger_photo_upload_enabled') !== 'false', // 預設開啟
...
export function setupSettings(state, db, renderApp) {
    ...
    // Setup Theme Switcher UI (透過模組化渲染)
    ...
    // Setup Budget Setting (移除全域預算，改為個別帳戶預算)
    // 預算邏輯已下放至 openAccountModal 內處理

    // Setup Accounts (多帳戶參數設定)
    const accountsContainer = document.getElementById('settings-accounts-container');
    const btnAddAccount = document.getElementById('btn-add-account');
    const ACCOUNT_COLORS = [
        { id: 'blue', name: '藍色', bgClass: 'bg-blue-50 dark:bg-blue-500/10', textClass: 'text-blue-600 dark:text-blue-400', borderClass: 'border-blue-200 dark:border-blue-500/30', badgeBg: 'bg-blue-600 dark:bg-blue-500 text-white' },
        { id: 'green', name: '綠色', bgClass: 'bg-green-50 dark:bg-green-500/10', textClass: 'text-green-600 dark:text-green-400', borderClass: 'border-green-200 dark:border-green-500/30', badgeBg: 'bg-green-600 dark:bg-green-500 text-white' },
        { id: 'red', name: '紅色', bgClass: 'bg-red-50 dark:bg-red-500/10', textClass: 'text-red-600 dark:text-red-400', borderClass: 'border-red-200 dark:border-red-500/30', badgeBg: 'bg-red-600 dark:bg-red-500 text-white' },
        { id: 'yellow', name: '黃色', bgClass: 'bg-yellow-50 dark:bg-yellow-500/10', textClass: 'text-yellow-600 dark:text-yellow-400', borderClass: 'border-yellow-200 dark:border-yellow-500/30', badgeBg: 'bg-yellow-600 dark:bg-yellow-500 text-white' },
        { id: 'purple', name: '紫色', bgClass: 'bg-purple-50 dark:bg-purple-500/10', textClass: 'text-purple-600 dark:text-purple-400', borderClass: 'border-purple-200 dark:border-purple-500/30', badgeBg: 'bg-purple-600 dark:bg-purple-500 text-white' },
        { id: 'gray', name: '灰色', bgClass: 'bg-slate-50 dark:bg-slate-500/10', textClass: 'text-slate-600 dark:text-slate-400', borderClass: 'border-slate-200 dark:border-slate-500/30', badgeBg: 'bg-slate-600 dark:bg-slate-500 text-white' }
    ];
    ...
    function saveAccounts(accounts) {
        localStorage.setItem('tinyledger_accounts', JSON.stringify(accounts));
        console.log('[設定] 更新帳戶設定（saveAccounts）成功');
    ...
    function renderAccounts() {
        ...
        accounts.forEach(account => {
            const colorDef = ACCOUNT_COLORS.find(c => c.id === account.color) || ACCOUNT_COLORS[0];
            const isDefaultBadge = account.isDefault ? `<span class="${colorDef.badgeBg}" style="font-size: 0.7rem; padding: 2px 6px; border-radius: 4px; margin-left: 8px;">預設</span>` : '';
            const setBtnHtml = !account.isDefault ? `<button class="btn-set-default" data-id="${account.id}" style="border: none; background: transparent; color: var(--text-muted); cursor: pointer; font-size: 0.8rem;">設為預設</button>` : '';
                ...
                try {
                    ...
                    if (boundTxs.length > 0 || boundFixed.length > 0) {
                        ...
                        if (boundTxs.length > 0) {
                            ...
                            const moreStr = dates.length > 5 ? ` 等共 ${dates.length} 個日期` : '';
                            msg += `- ${boundTxs.length} 筆單次交易紀錄 (存在於 ${displayDates}${moreStr})\n`;
                        }
                        
                        if (boundFixed.length > 0) {
                            msg += `- ${boundFixed.length} 筆固定支出/收入規則\n`;
                        }
    ...
    function openAccountModal(existingAccount) {
        ...
        if (!modal) {
            ...
            modal.innerHTML = `
                <div class="bg-surface rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-theme transform transition-all scale-100">
                    <div class="px-6 pt-6 pb-4 border-b border-theme flex items-center justify-between">
                        <h3 id="${modalId}-title" class="text-lg font-bold text-main tracking-tight">新增帳戶</h3>
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-main mb-1">帳戶名稱</label>
                            <input type="text" id="${modalId}-name" class="form-control w-full px-4 py-2" placeholder="例如: 現金、信用卡" maxlength="20">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-main mb-2">標籤顏色</label>
                            <div id="${modalId}-colors" class="flex flex-wrap gap-2">
                                ${ACCOUNT_COLORS.map(c => `<div class="color-swatch flex items-center justify-center w-8 h-8 rounded-full cursor-pointer transition-all ${c.bgClass} border-2 border-solid ${c.borderClass}" data-color="${c.id}"></div>`).join('')}
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-main mb-1">本月預算</label>
                            <input type="number" id="${modalId}-budget" class="form-control w-full px-4 py-2" placeholder="例如: 25000" min="0">
                        </div>
                    </div>
                    <div class="px-6 py-4 bg-surface border-t border-theme">
                        <button type="button" id="${modalId}-save" class="btn-primary w-full font-semibold py-2.5 rounded-lg transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1">儲存</button>
                    </div>
                </div>
            `;
            ...
            colorSwatches.forEach(sw => {
                sw.addEventListener('click', (e) => {
                    ...
                    e.currentTarget.innerHTML = `<span class="material-icons ${ACCOUNT_COLORS.find(c => c.id === e.currentTarget.dataset.color).textClass}" style="font-size: 16px;">check</span>`;
        ...
        colorSwatches.forEach(s => {
            if (s.dataset.color === targetColor) {
                const cDef = ACCOUNT_COLORS.find(c => c.id === targetColor) || ACCOUNT_COLORS[0];
        ...
        title.textContent = existingAccount ? '編輯帳戶' : '新增帳戶';
        
        // 強制提升層級並移到 DOM 樹最後方，避免被其它設定頁面遮蓋
    ...
    if (gasContainer) {
        ...
        window.gasBackupInstance = new GasBackupModule({
            ...
            onSaveSettings: () => {
                ...
                console.log('[設定] 更新雲端備份設定（onSaveSettings）成功');
            },
            ...
            onRestoreSuccess: async (payload) => {
                ...
                try {
                    ...
                    if (payload.preferences) {
                        ...
                        if (payload.preferences.accounts !== undefined) localStorage.setItem('tinyledger_accounts', payload.preferences.accounts);
                    }
                    ...
                    let catCount = 0, catSkipCount = 0, tgtCount = 0, tgtSkipCount = 0;
                    const existingTxs = await db.getTransactions();
                    const existingFixed = await db.getFixedRecords();
                    ...
                    if (wasEmpty) {
                        ...
                        if (txSkipCount > 0 || fixedSkipCount > 0 || catSkipCount > 0 || tgtSkipCount > 0) {
                            ...
                            if (catCount > 0) finalMsg += `- 類別設定：${catSkipCount} 個\n`;
    ...
    if (btnExport && !btnExport.hasAttribute('data-bound')) {
        ...
        btnExport.addEventListener('click', async () => {
            setAppBusy(true, { title: '正在匯出資料', detail: '準備下載 ZIP 備份檔...' });
            try {
                // 匯出時排除由固定紀錄自動產生的明細
                ...
                console.log(`[備份] 手動匯出地端備份（btn-export-json）成功: ZIP大小 ${Math.round(zipBlob.size/1024)} KB`);
            ...
            try {
                ...
                if (importedData.preferences) {
                    ...
                    if (importedData.preferences.customTheme) localStorage.setItem('tinyledger_custom_theme', importedData.preferences.customTheme);
                        ...
                        if (importedData.preferences.accounts !== undefined) localStorage.setItem('tinyledger_accounts', importedData.preferences.accounts);
                    ...
                    if (isOverwrite) {
                        setAppBusy(true, { title: '正在清空本機資料', detail: '刪除現有紀錄...', progress: 40 });
                        await db.clearTransactions();
                        await db.clearFixedRecords();
                        await db.clearCategories();
                        await db.clearTargets();
                    ...
                    if (importedData.targets && Array.isArray(importedData.targets)) {
                        ...
                        for (const tg of importedData.targets) {
                            if (!existingTargetNames.has(tg.name)) {
                                existingTargetNames.add(tg.name);
                                // 不刪除 tg.id，以保留原始資料的 ID（若使用者希望 ID 一致）
                    ...
                    console.log(`[備份] 從地端備份匯入（input-import-json）成功: 新增/更新 ${newTxs.length} 筆`);
    ...
    // 初始化系統日誌 (System Logs) UI
    mountSystemLogsUI({
        containerId: 'system-logs-module',
        description: '顯示最近 999 筆主控台日誌，方便排解連線或資料同步問題。'
    });
    ...
    const renderCatList = (container, cats, type) => {
        ...
        cats.forEach((c, catIdx) => {
            ...
            c.sub.forEach((sub, idx) => {
                ...
                li.innerHTML = `
                    <div style="display:flex; align-items:center; gap: 8px;">
                        <input type="checkbox" class="cat-checkbox cat-minor-checkbox" data-major="${c.major}" data-sub="${sub}" style="cursor:pointer;">
                        <span style="pointer-events: none;">${sub}</span>
                    </div>
                    <div style="display:flex; align-items:center; gap:4px;">
                        <button class="icon-btn edit-btn" data-major="${c.major}" data-sub="${sub}" title="編輯小類">
                            <span class="material-icons">edit</span>
                        </button>
                        <button class="icon-btn delete-btn" data-major="${c.major}" data-sub="${sub}" title="刪除小類">
            ...
            btn.addEventListener('click', async (e) => {
                ...
                if (cat) {
                    try {
                        ...
                        if (boundTxs.length > 0 || boundFixed.length > 0) {
                            let msg = '無法刪除！此類別已被使用於：\n';
                            
                            if (boundTxs.length > 0) {
                                const dates = [...new Set(boundTxs.map(t => t.date))].sort().reverse();
                                const displayDates = dates.slice(0, 5).join(', ');
                                const moreStr = dates.length > 5 ? ` 等共 ${dates.length} 天` : '';
                                msg += `- ${boundTxs.length} 筆單次交易紀錄 (發生於 ${displayDates}${moreStr})\n`;
                            }
                            
                            if (boundFixed.length > 0) {
                                const names = boundFixed.map(f => f.note || '無備註').slice(0, 3).join(', ');
                                const moreStr = boundFixed.length > 3 ? ` 等` : '';
                                msg += `- ${boundFixed.length} 筆固定支出/收入規則 (${names}${moreStr})\n`;
                            }
            ...
            btn.addEventListener('click', async (e) => {
                ...
                const saveEdit = async () => {
                    ...
                    if (isMajor) {
                        ...
                        // 小類別修改：限制同 type 與同 majorCategory
    ...
    if (btnDeleteSelectedCats && !btnDeleteSelectedCats.hasAttribute('data-bound')) {
        ...
        btnDeleteSelectedCats.addEventListener('click', async () => {
            ...
            let confirmMsg = `確定要刪除這 ${selectedMajors.length} 個大類、${allSelectedMinors.length} 個小類，共計 ${totalItems} 個項目嗎？\n(注意：刪除大類會一併刪除其下的所有小類)`;
            
            if (selectedMajors.length === 0) {
                confirmMsg = `確定要刪除這 ${allSelectedMinors.length} 個小類項目嗎？`;
            } else if (allSelectedMinors.length === 0) {
                confirmMsg = `確定要刪除這 ${selectedMajors.length} 個大類項目嗎？\n(注意：刪除大類會一併刪除其下的所有小類)`;
            }
            ...
            try {
                        ...
                        if (boundTxs.length > 0 || boundFixed.length > 0) {
                            ...
                            if (boundLines.length < 5) {
                                ...
                                if (boundTxs.length > 0) {
                                    ...
                                    details.push(`${boundTxs.length}筆紀錄(如${d})`);
                                }
                                if (boundFixed.length > 0) details.push(`${boundFixed.length}筆規則`);
                                boundLines.push(`- 小類 [${major} > ${subName}] (${details.join(', ')})`);
                            }
                        }
                    }
                }
                ...
                for (const cb of selectedMajors) {
                    ...
                    if (boundTxs.length > 0 || boundFixed.length > 0) {
                        ...
                        if (boundLines.length < 5) {
                            ...
                            if (boundFixed.length > 0) details.push(`${boundFixed.length}筆規則`);
                            boundLines.push(`- 大類 [${major}] (${details.join(', ')})`);
                        }
                    }
                }
                ...
                if (hasBound) {
                    ...
                    if (boundLines.length === 5) msgBound += '\n...及其他項目';
                    msgBound += '\n\n請先將這些紀錄刪除或修改為其他類別後再試。';
                    alert(msgBound);
                    return;
                }
            } catch (err) {
                console.error('Error checking category data in batch:', err);
            }
            ...
            btn.addEventListener('click', async (e) => {
                ...
                if (target) {
                    try {
                        ...
                        console.error('Error checking target data:', err);
                    }
        ...
        btnAddTarget.addEventListener('click', async () => {
            ...
            if (newName && newName.trim()) {
                ...
                if (state.targets.some(t => t.name === trimmedName)) {
                    alert(`對象名稱「${trimmedName}」已存在！`);
                    return;
                }
                
                const newTarget = { name: trimmedName, order: state.targets.length + 1 };
                const id = await db.saveTarget(newTarget);
                newTarget.id = id;
                state.targets.push(newTarget);
                renderTargetList();
    ...
    if (btnDeleteSelectedTargets && !btnDeleteSelectedTargets.hasAttribute('data-bound')) {
        ...
        btnDeleteSelectedTargets.addEventListener('click', async () => {
            ...
            try {
                    ...
                    if (target) {
                        ...
                        if (boundTxs.length > 0 || boundFixed.length > 0) {
                            ...
                            if (boundLines.length < 5) {
                                ...
                                if (boundFixed.length > 0) details.push(`${boundFixed.length}筆規則`);
                                boundLines.push(`- 對象 [${target.name}] (${details.join(', ')})`);
                            }
                        }
                    }
                }
                ...
                console.error('Error checking target data in batch:', err);
            }
