...
export function setupStatsView(state, db) {
    ...
    document.querySelectorAll('.stats-group-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            currentState.drillFilters = {}; // 清除下鑽狀態
            renderStats();
        });
    });
...
function getPeriodRange() {
    ...
    if (currentState.period === 'all') {
        return { start: '0000-01-01', end: now.toISOString().slice(0, 10) };
    }
    ...
    if (currentState.period === 'year') {
        ...
        const endDay = offset === 0 ? now.toISOString().slice(0, 10) : `${y}-12-31`;
    ...
    if (currentState.period === 'month') {
        ...
        let endDay = offset === 0 ? now.toISOString().slice(0, 10) : `${y}-${m}-${String(lastDayObj.getDate()).padStart(2, '0')}`;
    ...
    if (currentState.period === 'week') {
        ...
        let endDay = offset === 0 ? now.toISOString().slice(0, 10) : sunday.toISOString().slice(0, 10);
        return { start: monday.toISOString().slice(0, 10), end: endDay };
    }
}
...
export function renderStats() {
    ...
    if (currentState.drillFilters && Object.keys(currentState.drillFilters).length > 0) {
        allItems = allItems.filter(item => {
            for (const [k, v] of Object.entries(currentState.drillFilters)) {
                let itemVal = item[k];
                if (k === 'majorCategory' && !itemVal) itemVal = '(未分類)';
                if (k === 'subCategory' && !itemVal) itemVal = item.majorCategory || '(未分類)';
                if (k === 'payee' && !itemVal) itemVal = '(未指定)';
    ...
    // 統一更新上方的 GroupBtn UI，確保與 currentState.groupBy 狀態一致
    ...
    const renderChartSection = (title, colorClass, containerId, data, series = null) => {
        ...
        if (noData) {
            ...
            data.slices.forEach(s => {
                tableHTML += `
                    <tr class="stats-legend-row" data-label="${s.label}" style="border-bottom: 1px solid var(--border-color); cursor: pointer;" onmouseover="this.style.backgroundColor='var(--bg-color)'" onmouseout="this.style.backgroundColor='transparent'">
