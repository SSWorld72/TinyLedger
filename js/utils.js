    ...
    const addTx = (dateObj) => {
        ...
        if (dateStr >= dStrStart && dateStr <= dStrEnd) {
            txs.push({
                ...
                accountId: fixedRecord.accountId || 'account_default'
            });
        }
    };
...
/**
 * Generate a fingerprint for a normal transaction, used to detect duplicates
 */
export const getTxFingerprint = (tx) => {
    const accountId = tx.accountId || 'account_default';
    return `${accountId}_${tx.date}_${tx.type}_${tx.majorCategory}_${tx.subCategory}_${tx.amount}_${tx.payee || ''}_${tx.location || ''}_${tx.note || ''}`;
};

/**
 * Generate a fingerprint for a fixed record, used to detect duplicates
 */
export const getFixedFingerprint = (f) => {
    const accountId = f.accountId || 'account_default';
    const ruleStr = typeof f.ruleDetail === 'object' ? JSON.stringify(f.ruleDetail) : f.ruleDetail;
    return `${accountId}_${f.startDate}_${f.endDate}_${f.rule}_${ruleStr}_${f.type}_${f.majorCategory}_${f.subCategory}_${f.amount}_${f.payee || ''}_${f.location || ''}_${f.note || ''}`;
};
